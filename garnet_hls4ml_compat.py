...
class GarNet(keras.layers.Layer):
    def __init__(self, n_aggregators, n_filters, n_propagate,
        ...
        self._kernel_quant = kernel_quant
    ...
    def _setup_transforms(self, n_aggregators, n_filters, n_propagate):
        if self._quantize_transforms:
            self._input_feature_transform = NamedQDense(n_propagate, kernel_quantizer=self._kernel_quant, bias_quantizer=self._bias_quant, name='FLR')
            self._output_feature_transform = NamedQDense(n_filters, activation=self._output_activation, kernel_quantizer=self._kernel_quant, name='Fout')
        else:
    ...
    def _add_transform_config(self, config):
        config.update({
            ...
            'kernel_quant': self._kernel_quant,
...
class GarNetStack(GarNet):
    ...
    def _setup_transforms(self, n_aggregators, n_filters, n_propagate):
        ...
        for it, (p, a, f) in enumerate(zip(n_propagate, n_aggregators, n_filters)):
            if self._quantize_transforms:
                input_feature_transform = NamedQDense(p, kernel_quantizer=self._kernel_quant, bias_quantizer=self._bias_quant, name=('FLR%d' % it))
                output_feature_transform = NamedQDense(f, activation=self._output_activation, kernel_quantizer=self._kernel_quant, name=('Fout%d' % it))
            else:
